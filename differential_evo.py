...
def diff_evo(matrix_of_growth_dynamics, K, states, drugs_considered, state_associations, death_rate,
    ...
    bounds = [(lower_bound, upper_bound)] * treatments
    print(bounds)
    ...
    best_sequence = result.x.astype(int)
...
def run_optimal_(matrix_of_growth_dynamics, K, states, drugs_considered, state_associations, death_rate,
    ...
    for drug_index in best_sequence:  # drugs to iterate over
        ...
        solution = solve_ivp(ODE_solve, y0=x_i, t_span=[0, t_cycle],
                             args=(matrix_i, K, death_rate, mu, mutation_state_i),
                             t_eval=t_eval)
        # Keep track of time
    ...
    #todo: output data for total population
...
def system_dynamics_int(F, t, R, M, Delta_t, sequence, antibiotics, K, death_rate, mu):
    idx = int(t / Delta_t)
    ...
    A = R[ind] * (1 - (np.sum(F) / K)) - death_rate * np.eye(len(F)) + mu * M[ind]
    dFdt = np.dot(A, F)
    return dFdt


def objective_function_int(sequence, F0, times, R, M, Delta_t, bactTypes, antibiotics, K, death_rate, mu):
    # Solving the differential equations
    fSol = odeint(system_dynamics_int, F0, times, args=(R, M, Delta_t, sequence, antibiotics, K, death_rate, mu))
    X = 0
