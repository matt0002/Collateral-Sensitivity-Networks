...
app.layout = (html.Div(children=[
    ...
    html.Div(children=[
        html.H4(children='Enter mutant growth rate'),
    ...
    html.Div(id='div_for_ODE', style={'width': '80%', 'display': 'inline-block'})
...
def ODE(*vals):
    drug1, drug2, drug3, drug_4, mutant_growth, susceptible_growth, death_rate, cycle_time, run = vals
    if run == 0:
        raise exceptions.PreventUpdate
    else:
        ODE_graph = diff_eq(final_graphs, drug1, drug2, drug3, drug_4, float(mutant_growth), float(susceptible_growth),
                            float(death_rate), int(cycle_time))
        return dcc.Graph(figure=ODE_graph)
