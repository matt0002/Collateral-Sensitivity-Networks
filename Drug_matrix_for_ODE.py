...
def ODE_for_Switching_systems(matrix_of_growth_dynamics, K, states, drugs_considered, state_associations, death_rate,
    ...
    for cycle_drug in range(number_of_cycles):  # total simulation time
        for drug_index, drug_element in enumerate(drugs_considered):  # drugs to iterate over
            ...
            solution = solve_ivp(ODE_solve, y0=x_i, t_span=[0, t_cycle],
                                 args=(matrix_i, K, ro_Death, ro_Mutation, mutation_state_i, persister_pop),
                                 t_eval=t_eval)
            # Keep track of time
    ...
    fig.update_layout(
        ...
        plot_bgcolor='white',
        xaxis_title='Time (hours)',
        title=f'Simulation of Sequential treatment')
...
def ODE_solve(t, y, final_matrix, K, ro_Death, mutation_rate, mutation_states, persist_pop):
    x_i = y
    ...
    logistic = np.matrix([(1 - ((total_pop + persist_pop) / K)) for i in range(len(x_i))]).transpose()
    logistic = np.array(logistic)

    # growth rate multiplied by logistic
